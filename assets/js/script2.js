let preguntas2 = [
  ...
  },
  {
    ...
    opcion: [
      "A.Lleva a cabo una función para establecer las diferencias en los orígenes de datos de Git. ",
      "B.Para revisar cuáles archivos se encuentran en el staged.",
      "Para inicializar un proyecto git.",
      "Git diff no es un comando de git.",
    ],
    respuesta: 1,
  ...
  {
    pregunta:
      "Tipo de dato que permite almacenar cadenas de texto (En javascript):",
  ...
  },
  {
    ...
    opcion: [
      "A.Git branch -r",
      "B.Git branch",
      "C.Git branch origin remote",
      "D.Git checkout branch remote",
    ],
    respuesta: 1,
  ...
  {
    pregunta:
      "Comando usado para para mostrar información sobre cualquier objeto git.",
  ...
  {
    pregunta:
      "Comando empleado para extraer y descargar contenido desde un repositorio remoto y actualizar al instante el repositorio local para reflejar ese contenido.",
  ...
  {
    pregunta:
      "Git es un sistema de control de versiones distribuido que fue creado por Linus Torvalds en el año:",
...
function validar(opcion) {
  ...
  while (repetir) {
    if (op1 == preguntas[index].respuesta) {
      ...
      a++;
      alert(
        `Lo sentimos ha seleccionado una respuesta erronea. El dinero ganado es de $0 y su puntaje es de ${puntaje}`
      );
      mostrarResultado2();
      break;
    }
...
function botoncincuenta(opcion) {
  while (repetir2) {
    ...
    if (
      span1.classList.contains("correcto") ||
      span2.classList.contains("correcto") ||
      span3.classList.contains("correcto") ||
      span4.classList.contains("correcto")
    )
      return alert(
        "No puedes pedir el comodin porqye ya tienes una opción correcta"
      );
  ...
  alert("Ha usado su comodin mi amigo, ya no podrá utilizarlo nuevamente");
}
...
llamada.addEventListener("click", () => {
  if (
    ...
    return alert(
      "UPS parece que un zombie te ha mordido , no puedes pedir el comodin pues ya tienes una opcion correcta"
    );
...
finalizar.addEventListener("click", () => {
  let seguro = prompt(
    "¿Esta seguro que desea retirarse?, digite si para confirmar , de lo contrario cierre o pulse cualquier otra tecla para continuar"
  );
  seguro.toLowerCase();
...
function mostrarResultado2() {
  ...
  correctiyo.innerHTML = `Perdiste al intento ${puntaje}, ¿puedes esforzarte más?`;
}
